...
func main() {
	...
	var gameEntities []*ecs.IEntity = []*ecs.IEntity{
		ecs.CEntityPossessed(&gameWorld, uuid.New(), newUUID, []*ecs.IComponent{
	...
	serverEngine.Start(
		...
		func(masterConnexion *net.UDPConn, addr *net.UDPAddr) {
			...
			for {
				...
				switch handleMessage.MessageType {
				case "CONNECT":
					messageData, ok := handleMessage.Data.(map[string]interface{})
					if !ok {
						bytes, err := engine.MessageToBytes(messages.SC_CONNECT_FAILED("data n'est pas un objet"))
						if err != nil {
							log.Println(err.Error())
				...
				case "CONNECT_TOKEN":
					// We bind the connexion to the Client
					tokenString, ok := handleMessage.Data.(string)
					if !ok {
						log.Println("CONNECT_TOKEN Could not parse data to string")
					}
				...
				case "MOVE":
					direction := handleMessage.Data.(string)
					...
					if foundId != nil {
						...
						if entities := gameWorld.GetEntitiesPossessedBy(*foundId); len(entities) != 0 {
							for _, entity := range entities {
								...
								positionComponentLocalised.SetData(positionComponentDataParsed)
